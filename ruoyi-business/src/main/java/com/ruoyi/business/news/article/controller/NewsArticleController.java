...
@RestController @RequestMapping("/business/news/article") public class NewsArticleController extends BaseController {
 ...
 @PreAuthorize("@ss.hasPermi('business:news:process:list')") @GetMapping("/list") public TableDataInfo list(NewsArticle query){startPage(); List<NewsArticle> list=mapper.selectList(query);return getDataTable(list);}
 @PreAuthorize("@ss.hasPermi('business:news:process:query')") @GetMapping("/{id}") public AjaxResult get(@PathVariable Long id){return success(mapper.selectById(id));}
}

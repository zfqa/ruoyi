<template>
  <div class="app-container">
    ...
    <el-alert
      title="采集成功后仅暂存结果，主库统计与统一知识库都不会自动写入。请点击「本次全部入库」或单条「入库」后同步更新主库侧并进入知识库。"
      type="info"
      :closable="false"
      show-icon
      class="mb12"
  ...
  methods: {
    ...
    publishTask(row) {
      ...
      publishTaskNewsKnowledge(row.id).then(r => {
        const data = r.data || {}
        this.$modal.msgSuccess(`已处理${data.total || 0}条：新入库${data.submitted || 0}条，已存在${data.reused || 0}条，失败${data.failed || 0}条；主库新增${data.mysqlInserted || 0}/更新${data.mysqlUpdated || 0}/已有${data.mysqlExisting || 0}`)
        this.load()
    ...
    publishArticle(article) {
      ...
      publishArticleKnowledge(this.viewingTaskId, article.articleId).then(r => {
        const data = r.data || {}
        this.$modal.msgSuccess(data.reused ? '该新闻已在知识库中，已复用现有版本；主库结果已同步' : '新闻已提交知识库入库，主库结果已同步')
        this.load()
    ...
    deleteSelectedArticles() {
      const articleIds = this.selectedArticles.map(item => item.articleId)
      if (!articleIds.length) return
      const removeAll = articleIds.length >= this.taskArticles.length
      const tip = removeAll
        ? '将移除本次全部新闻，并同步删除该采集任务，是否继续？'
        : '确认移除选中的任务新闻？'
      this.$modal.confirm(tip).then(() => request({
        url: '/business/news/collect/' + this.deletingTaskId + '/articles/delete',
        method: 'post',
        data: { articleIds }
      })).then(r => {
        const data = (r && r.data) || {}
        this.deleteDialog = false
        this.articlesDialog = false
        if (data.taskDeleted) {
          this.$modal.msgSuccess('已移除全部新闻，采集任务已同步删除')
          this.viewingTaskId = null
          this.deletingTaskId = null
          this.taskArticles = []
        } else {
          this.$modal.msgSuccess('已移除选中的任务新闻')
          if (this.deletingTaskId) this.viewTaskArticles(this.deletingTaskId)
        }
        this.load()
      })
    },
    showArticle(article) { this.selectedArticleId = article.articleId; this.selectedOperation = article.operation; this.selectedMysqlOperation = this.mysqlOpLabel(article.mysqlOperation); this.articleDrawer = true },
    dateRange(row) { return row.publishTimeStart || row.publishTimeEnd ? (row.publishTimeStart || '不限') + ' ~ ' + (row.publishTimeEnd || '不限') : '不限发布时间' },
    mysqlOpLabel(op) {
      const map = {
        PENDING_INSERTED: '待入库(新增)',
        PENDING_UPDATED: '待入库(更新)',
        PENDING_EXISTING: '待入库(已有)',
        MYSQL_INSERTED: '已入库(新增)',
        MYSQL_UPDATED: '已入库(更新)',
        MYSQL_EXISTING: '已入库(已有)'
      }
      return map[op] || op || '-'
    }
  }
...
<style scoped>.mb12{margin-bottom:12px}.source-select{width:300px}.empty-tip{padding:18px 0;text-align:center;color:#909399}.history-stat{color:#909399}.danger{color:#f56c6c}.delete-tip{margin-bottom:12px;color:#909399}</style>
